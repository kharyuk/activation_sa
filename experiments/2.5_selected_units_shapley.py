...
if do_compute_values:
    ...
    for channel_name in preparation.single_unit._HSVG_channel_names:
        ...
        shpv_fnm = f'{channel_name}_{values_fnm_prefix}_{fnm_suffix}.hdf5'
        shpv_path = os.path.join(activations_dirname, shpv_fnm)
        
        log_fnm = f'{channel_name}_{values_fnm_prefix}_{fnm_suffix}.log'
        log_path = os.path.join(activations_dirname, log_fnm)
        
        current_shpv_path = sensitivity_analysis.shapley.analyze_transform_with_shapley(
            #network,
            network_modules,
            activations_path,
            #variables_sigmas,
            ...
            verbose=True,
            rewrite_hdf5=True,
            tmp_save_path=f'../results/tmp/2.5/{augmentation_set_number}/',
            activations_key=channel_name
        )
        shpv_paths.append(current_shpv_path)
        print(f'Please find the computed results in file: {current_shpv_path}')
