...
arg_parser.add_argument(
    '--Npermutations', metavar='Npermutations', type=int, default=2**8,
    help='Number of permutations to estimate total variance.'
)
...
######################################################
#### 7. compute the Mean/Variances for augmentation scheme 2
...
if do_compute_values:
    ...
    tv_path = sensitivity_analysis.totvar.compute_total_variances(
        ...
        verbose=True,
        rewrite_hdf5=True,
        tmp_save_path=f'../results/tmp/1.4/{augmentation_set_number}/',
    )
