...
if do_compute_values:
    ...
    for channel_name in preparation.single_unit._HSVG_channel_names:
        ...
        tv_fnm = f'{channel_name}_{values_fnm_prefix}_{fnm_suffix}.hdf5'
        tv_path = os.path.join(activations_dirname, tv_fnm)

        current_tv_path = sensitivity_analysis.totvar.compute_total_variances(
            #network,
            network_modules,
            activations_path,
            log_path=log_path,
            ...
            verbose=True,
            rewrite_hdf5=True,
            tmp_save_path=f'../results/tmp/2.2/{augmentation_set_number}/',
            activations_key=channel_name
        )
        tv_paths.append(current_tv_path)
        print(f'Please find the computed results in file: {current_tv_path}')
