...
arg_parser.add_argument(
    ...
    type=str,
    default='imagenet',
    help='which dataset to work with [default: ImageNet/ILSVRC]'
)
arg_parser.add_argument(
    '--model_filename',
    type=str,
    default='',
    help='Filename of pretrained model parameters. Default: empty, use pretrained Imagenet models'
)
arg_parser.add_argument(
...
arg_parser.add_argument(
    '--si_analyze_random_state',
    type=int,
    default=123,
    help='random permutation state for dataset'
)
arg_parser.add_argument(
    '--class_sampler_seed',
    type=int,
...
arg_parser.add_argument(
    ...
    type=str,
    default='si_alexnet_imagenet_ILSVRC_activations',
    help='name prefix for hdf5 file containing the activations'
)
arg_parser.add_argument(
    '--values_fnm_prefix',
    type=str,
    default='si_alexnet_imagenet_ILSVRC_values',
...
arg_parser.add_argument(
    ...
    default=2**14,
    help=(
        'Number of samples to compute Sobol indices. Note that actiual number is dependent on'
        ' number of variables and whether compute 2nd order indices or not'
    )
)
arg_parser.add_argument(
    '--Njobs_si_computing',
...
arg_parser.add_argument(
    ...
    type=int,
    default=1,
    help='whether use permutation variable or not'        
)
arg_parser.add_argument(
    '--use_switch_variables',
    type=int,
    default=0,
    help='whether use switchers for transformations or not'        
)
arg_parser.add_argument(
    '--compute_s2',
    type=int,
    default=0,
    help='whether compute the 2nd order Sobol indices or notaaz'        
)
...
arg_parser.add_argument(
    '--json_config_path',
    type=str,
    help='path to json file which contains the config_dicts for every subunits to be analyzed'
)
...
buffer_size_si_computing = args.buffer_size_si_computing
Njobs_si_computing = args.Njobs_si_computing

si_analyze_random_state = args.si_analyze_random_state
...
if dataset_name == 'imagenet':
    ...
    dataset_loader_func = data_loader.places365.CustomPlaces365
else:
    raise ValueError
...
config_list = preparation.single_unit.load_config_from_json(json_config_path)
config_list = preparation.single_unit.extract_neuron_indices(config_list)
units = preparation.single_unit.get_units_of_interest(network, config_list)

# assumption: type(units[x][0]) = conv2d 
#in_channels_list = [units[x][0].in_channels for x in units]

def get_module_by_type(module, mtype=nn.Conv2d):
    ...
    for child in module.children():
        result = get_module_by_type(child, mtype)
...
in_channels_list = [
    get_module_by_type(units[x], mtype=nn.Conv2d).in_channels for x in units
]
print(in_channels_list)

common_inference_module = preparation.single_unit.SimultaneousInferenceSingleChannelModule(
    units, in_channels_list
)
common_inference_module.eval();

network_modules = list(
    map(
        preparation.single_unit.get_network_module_name, config_list
    )
)
...
(
    ...
    use_switch_variables=False, #use_switch_variables
...
if do_compute_activations:
    ...
    if (not actfnm_isin_dir) or (actfnm_isin_dir and recompute_activations):
        ...
        activations_path = sensitivity_analysis.sobol.compute_activations_wrt_sobol_indices(
            ...
            custom_save_activations_function=preparation.single_unit.save_single_channel_activations
        )

######################################################
#### 7. compute the Sobol indices [S1, ST, optionally S2]
...
if do_compute_values:
    ...
    for channel_name in preparation.single_unit._HSVG_channel_names:

        log_fnm = f'{channel_name}_{values_fnm_prefix}_{fnm_suffix}.log'
        log_path = os.path.join(activations_dirname, log_fnm)

        si_fnm = f'{channel_name}_{values_fnm_prefix}_{fnm_suffix}.hdf5'
        si_path = os.path.join(activations_dirname, si_fnm)

        current_si_path = sensitivity_analysis.sobol.analyze_transform_with_sobol(
            problem,
            network_modules,
            activations_path,
            value_names,
            ...
            log_path=log_path,
            si_path=si_path,
            Njobs=Njobs_si_computing,
            buffer_size=buffer_size_si_computing,
            seed=si_analyze_random_state,
            class_variable=True,
            verbose=True,
            rewrite_hdf5=True,
            tmp_save_path=f'../results/tmp/2.3/{augmentation_set_number}/',
            activations_key=channel_name
        )
        si_paths.append(current_si_path)
        print(f'Please find the computed results in file: {current_si_path}')
