...
arg_parser.add_argument(
    '--permutation_random_seed', metavar='permutation_random_seed', type=int, default=577,
    help='Random seed for sampling permutatons'
)
arg_parser.add_argument(
    '--problem_sampler_seed', metavar='problem_sampler_seed', type=int, default=233,
...
common_inference_module = preparation.single_unit.SimultaneousInferenceModule(units)

network_modules = list(map(lambda x: x['unit_name'], config_list))
...
if do_compute_activations:
    ...
    if (not actfnm_isin_dir) or (actfnm_isin_dir and recompute_activations):
        activations_fnm = sensitivity_analysis.shapley.compute_activations_wrt_total_variance(
            sampler,
...
######################################################
#### 7. compute the 2nd sampling scheme's total variances
...
if do_compute_values:
    ...
    for channel_name in preparation.single_unit._HSVG_channel_names:
        ...
        current_tv_path = sensitivity_analysis.totvar.compute_total_variances(
            ...
            verbose=True,
            rewrite_hdf5=True,
            tmp_save_path=f'../results/tmp/2.4/{augmentation_set_number}/',
