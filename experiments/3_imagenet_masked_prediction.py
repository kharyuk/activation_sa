...
log_fnm = f'{sensitivity_values_name}_{predictions_fnm_base}_{fnm_suffix}.log'
log_path = os.path.join(sensitivity_values_dirname, log_fnm)

trace_fnm = f'{sensitivity_values_name}_{predictions_fnm_base}_params_{fnm_suffix}.npz'
trace_sampled_params_path = os.path.join(sensitivity_values_dirname, trace_fnm)

results_fnm = f'{sensitivity_values_name}_{predictions_fnm_base}_{fnm_suffix}.hdf5'
results_path = os.path.join(sensitivity_values_dirname, results_fnm)
print(results_fnm)
print(device)

os.makedirs(sensitivity_values_dirname, exist_ok=True)
resfnm_isin_dir = os.path.isfile(results_path)

if resfnm_isin_dir and recompute_predictions:
    os.remove(results_path)
    resfnm_isin_dir = False

if resfnm_isin_dir:
    print(f'HDF5 file {results_path} already exists')
else:
    # filtered_network_leaves without the 'model_outputs', save_output_activations controls it here
    predictions_path = prediction.compute.compute_predictions_wrt_single_augs(
        sampler,
        network,
