...
The second dataset can be found at either its homepage or a Kaggle upload:
- http://places2.csail.mit.edu/
- https://www.kaggle.com/datasets/benjaminkz/places365

We provide the downloading and tuning scripts as a part of this repository. Please use the ```1-1_Download_ILSVRC_imagenet_data.ipynb``` and ```1-1_Download_Places365_data.ipynb``` (Kaggle account's username and key are required).

Also these notebooks provides user with scriots to download the pretrained models used in this study:
- ```alexnet-owt-4df8aa71.pth``` (Imagenet; 244.4 MB)
- ```vgg11-bbd30ac9.pth``` (Imagenet; 531.5 MB)
...
For user's convenience, we separately uploaded resulting scripts into ```scripts``` directory.
...
- **Preparation**:
    - ```1-1_Download_<dataset_name>_data.ipynb```: download data and pretrained models;
    - ```1-2_Display_<dataset_name>_data.ipynb```: observe data and augentations considered in the study;
...
- **Experimental series 3 (single channelled segments)**:
    ...
    - ```<network_name>_single_units.json```: these files define the parts of the networks to be analyzed in the 3rd experimental series.
